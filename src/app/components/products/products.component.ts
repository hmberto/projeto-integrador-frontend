...
export class ProductsComponent implements OnInit {
  ...
  get drogariaSpProducts(): Product[] {
    return this.productService.products.filter((product) => product.pharmacy === 'Drogaria São Paulo' && product.pharmacyId === '14');
  }

  get ultrafarmaProducts(): Product[] {
    return this.productService.products.filter(product => product.pharmacy === 'Ultrafarma' && product.pharmacyId === '24');
  }

  get pagueMenosProducts(): Product[] {
    return this.productService.products.filter(product => product.pharmacy === 'Pague Menos' && product.pharmacyId === '34');
  }

  get drogaRaiaProducts(): Product[] {
    return this.productService.products.filter(product => product.pharmacy === 'Droga Raia' && product.pharmacyId === '4');
  }
